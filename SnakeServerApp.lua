...
function SnakeServerApp:OnSnakeMoveReq(player, msg)
	...
	if self:CheckMove(dir, x,y) == true then
...
function SnakeServerApp:CheckMove(dir, x,y)
...
function SnakeServerApp:SendChangeWall(i,j)
...
function SnakeServerApp:CheckAttack(snake_id, dir, x, y)
	...
	if dir == "up" then
		...
		if inter == true then
			self:SendChangeWall(i,j-1)
		end
		inter = self:IntersectRect(attack_rect,i-1,j-1)
		if inter == true then
			self:SendChangeWall(i-1,j-1)
		end
		inter = self:IntersectRect(attack_rect,i+1,j-1)
		if inter == true then
			self:SendChangeWall(i+1,j-1)
